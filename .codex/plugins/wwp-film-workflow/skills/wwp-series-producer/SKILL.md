...
1. Confirm series title, season, input directory, intended spec, and strong series evidence such as `SxxEyy`, episode count, first-air metadata, or a TMDb `/tv/` identity before creating the ledger work. For a multi-season source, split and identify every season before creating any destination pages; create/reuse one database work entry for each verified season, rather than ordinary child pages below the parent series. Create/reuse the canonical page with `tools/notion-create-work-page.mjs --work-id <id>` and immediately read back `影别`: normal series/season work must be `TV Series` (not `Movie`) before any spec, Episode, release, or website-sync action. Correct a mismatch through the same work-page tool and retain the page hidden until the normal release gates pass.
