...
6. Classify each eligible movie as standard-value or high-value internally. First optimize batch coverage: give every eligible newly arrived work one releaseable playable before expanding a covered work. For standard-value work, a compact playable is normally the first and possibly final tier. For high-value work, evaluate compact, balanced, and preservation-oriented versions independently, but separate the first-release choice from later expansion. Create only source-supported, non-duplicate variants. Spec titles contain only verifiable media facts. For series, set the per-episode size range from actual QC-passed outputs in decimal GB, never a pre-encode estimate.
   For a foreign-language work, make this bitrate decision on the original-language branch first. A dubbed release may improve accessibility, but it does not satisfy or close original-audio coverage.
...
For foreign films, apply the matrix in this order when the source supports it: compact original audio with Chinese subtitles; higher-bitrate original audio for high-value work; then useful dubbed or alternate-audio supplements. Children's/family access needs may move Mandarin earlier, but never remove the explicit original-audio decision. If a dubbed high tier exists while the original high tier does not, record the original high variant as selected/deferred or record the exact source/QC/subtitle blocker.
