...
2a. Immediately read back a newly created/reused page and compare `影别` with the ledger work type: `movie` must be `Movie`; `series` must be `TV Series`. Correct a mismatch before any metadata or child-page work, and record it as an identity/structure correction rather than allowing a series to inherit movie defaults.
...
17. After a metadata-only pass, preserve `Workflow Status=已完成` only if the separate playable cycle already has exact `sync_ready` evidence. If the current source cannot continue to playable production, set `Workflow Status=暂缓` and append an AI marker naming the blocker and the condition that would reopen it.
