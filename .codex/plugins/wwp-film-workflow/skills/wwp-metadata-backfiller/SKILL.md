...
3. Run identity maintenance for `WW Work ID`, parsed IDs, title-derived fields, and schema readiness.
4. Use explicit IDs when available, especially Douban Subject ID and IMDb ID.
5. Fetch missing fields from the appropriate source and record source/status.
6. Fill the current structured field set, not only `基本信息`: `Release Year`, `上映日期`, `Countries`, `Languages`, `Traditional Chinese Title (Taiwan)`, `Traditional Chinese Title (Hong Kong)`, `旨趣`, `外部类型原文`, `未映射类型`, `Runtime Minutes`, `Directors`, `Writers`, `Cast`, ratings, poster fields, IDs/URLs, `Match Status`, `Metadata Status`, `Metadata Source`, `Metadata Confidence`, and `Metadata Updated At`.
7. Run OMDb after IMDb ID exists to fill `分级`, `Metascore`, `烂番茄新鲜度`, `Box Office`, `Box Office Amount`, `Box Office Currency`, and `Box Office Source`.
8. Use AI only for advisory/generated fields such as `AI建议最低年龄`, `AI年龄建议置信度`, `内容风险标签`, `AI年龄建议理由`, family viewing notes, or summary phrasing; do not invent external facts.
9. Apply patches conservatively and verify readback.
