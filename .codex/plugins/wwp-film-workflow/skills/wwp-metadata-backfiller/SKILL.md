...
2a. Determine `movie` versus `series` before the first Notion metadata write. Use strong identity evidence such as a TMDb `/movie/` or `/tv/` URL, explicit episode count/first-air wording, and verified `SxxEyy` source structure; write that decision to the ledger. Create/reuse the Notion page only with `tools/notion-create-work-page.mjs --work-id <id>` so `影别` is derived from the ledger rather than a CLI default. Immediately read the page back: `movie` must be `Movie`; `series` must be `TV Series`. Treat missing/conflicting evidence as an intake blocker, and treat a ledger/Notion mismatch as an identity incident that blocks metadata completion, child-page work, publication reconciliation, and website-release completion.
...
14. Apply patches conservatively and verify the exact page by readback. Run the
    metadata completeness calculation after the sourced and AI passes; do not
    infer completion from the command exit code or from fields attempted.
15. Treat metadata completeness as a core-field gate, not an instruction to invent every optional value. Missing TMDb, regional title, official rating, critic score, box office, `未映射类型`, or `人工年龄覆盖` is acceptable when the source does not provide it. Empty `未映射类型` means all observed genres mapped successfully; empty `人工年龄覆盖` means no human override. Promote to `verified` only when core identity/descriptive/poster/AI advisory fields are present, at least one verified external identity exists, `Human Issue` plus `AI Issue` are empty, and an exact readback confirms those facts. A poster counts only when the maintained poster field supplies a URL/file that can be fetched for website caching; an empty `海报` file field is acceptable only when the maintained `Poster URL` is usable and later website readback succeeds.
15a. Complete the ledger `metadata_backfill` task only after that exact readback
     reports `Metadata Status=verified`. If the result is `partial`, keep it
     pending while another deterministic source or AI pass is available. If no
     current source can fill the missing core fields, defer it with the exact
...
- `Hide from Website` is a workflow-owned visibility safety gate. Metadata tools do not clear it; the publisher clears it only after all playback and publication evidence passes.
