...
- `node tools/film-ledger.mjs retire-variant --variant <id> --failure-code <code> --failure-detail <reason> --json`
  - Retires an already QC-passed output when the user explicitly cancels that optional specification before publication. Confirm that no upload is in flight, archive only an exact empty placeholder page when one was created for it, then retire the variant so it leaves publication and handoff queues. This is not a substitute for a temporary upload failure or a pending human decision.
