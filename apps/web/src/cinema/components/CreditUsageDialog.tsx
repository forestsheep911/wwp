  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:w-[min(94vw,720px)]">
        ...
        {error ? <p className="rounded-xl border border-rose-400/30 bg-rose-400/10 px-3 py-2 text-sm font-semibold text-rose-200 sm:rounded">{error}</p> : null}

        {loading ? (
          <div className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950/70 px-4 py-6 text-sm font-semibold text-slate-300 sm:rounded">
            <Loader2 className="h-4 w-4 animate-spin" />
            {copy.credit.loadingUsage}
          </div>
        ) : entries.length === 0 ? (
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 px-4 py-6 text-sm text-slate-400 sm:rounded">
            {copy.credit.noUsage}
          </div>
        ) : (
          <div className="grid gap-3">
            <div className="grid gap-2 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 sm:rounded">
            ...
            <div className="max-h-[52dvh] overflow-auto rounded-xl border border-slate-800 sm:rounded">
              <div className="grid gap-2 p-2 sm:gap-0 sm:divide-y sm:divide-slate-800 sm:p-0">
                {entries.map((entry) => (
                  <div key={entry.id} className="grid gap-2 rounded-lg bg-slate-950/50 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:rounded-none">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-slate-50" title={entry.title}>{entry.title}</p>
                      <p className="mt-1 break-all font-mono text-xs leading-5 text-slate-500 sm:truncate" title={entry.assetKey}>{entry.assetKey}</p>
