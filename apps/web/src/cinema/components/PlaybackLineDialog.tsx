import { CheckCircle2, Globe2, Loader2, MapPin, Play, Sparkles } from "lucide-react";
...
export function PlaybackLineDialog({
  ...
  return (
    <Dialog open={Boolean(choice)} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:w-[min(94vw,620px)]">
        <DialogHeader>
          <div className="mb-1 flex items-center gap-2 text-emerald-300">
            <Play className="h-4 w-4 fill-current" />
            <span className="text-xs font-black tracking-[0.18em]">在线播放</span>
          </div>
          <DialogTitle>这次使用哪条线路？</DialogTitle>
          <DialogDescription>
            只影响这一次播放，不会切换片库或页面内容。
          </DialogDescription>
        </DialogHeader>
        ...
        {choice?.loading ? (
          <div className="flex min-h-40 items-center justify-center gap-2 rounded-xl border border-slate-800 bg-slate-950/70 text-sm font-semibold text-slate-300">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(["domestic", "international"] as const).map((line) => {
              ...
              const enabled = playable || Boolean(choice?.canPrepare);
              const isPreferred = preferredLine === line;
              return (
                <button
                  className={`group relative grid min-h-44 gap-4 rounded-xl border p-4 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:cursor-not-allowed disabled:border-slate-800 disabled:bg-slate-950/50 disabled:opacity-55 ${detail.accent}`}
                  disabled={!enabled}
                  key={line}
                  type="button"
                ...
                >
                  ...
                  <div className={`flex items-center gap-1.5 text-xs font-bold ${
                    ...
                    {playable ? <CheckCircle2 className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                    {playable
                      ? "已准备，直接播放"
                      : choice?.canPrepare
                        ? "尚未准备，选择后开始准备"
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-slate-800 pt-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs leading-5 text-slate-500">系统会记住上次选择，仅作为下次的醒目标记。</p>
