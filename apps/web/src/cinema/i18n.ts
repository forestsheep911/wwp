...
export const zhCN = {
  ...
  watchlist: {
    title: "今晚看什么",
    description: "从已缓存影片里挑一部，按心情、轻重和最近观看情况缩小选择。",
    available: (count: number) => `${count} 部可选`,
    unseen: (count: number) => `${count} 部没看过`,
    rewatch: (count: number) => `${count} 部可重温`,
    pick: "随机挑一部",
    picked: "今晚就它",
    filters: {
      all: "全部",
    ...
    neverPlayed: "还没看过",
    lastPlayed: (date: string) => `上次看 ${date}`,
    cachedAt: (date: string) => `缓存 ${date}`,
    requestedAt: (date: string) => `请求 ${date}`,
    source: (name: string) => `片源 ${name}`,
    quality: "播放状态",
    play: "播放",
    refresh: "刷新片单",
    reasonFresh: "适合开新片",
