...
function planForPage(page: JsonRecord): CleanupPlan {
  ...
  const chineseTitle = textFromProperty(properties["Chinese Title"]);
  const simplifiedChineseTitle = textFromProperty(properties["Simplified Chinese Title"]);
  let action: CleanupPlan["action"] = "empty";

  if (chineseTitle && !simplifiedChineseTitle) {
    action = "migrate";
  } else if (chineseTitle && simplifiedChineseTitle && chineseTitle !== simplifiedChineseTitle) {
    action = "conflict";
  } else if (chineseTitle && simplifiedChineseTitle) {
  ...
  return {
    ...
    simplifiedChineseTitle: simplifiedChineseTitle || undefined,
    action
  };
}

async function archiveChineseTitleProperty(notion: Client, dataSourceId: string) {
  const attempts: Array<{ method: string; properties: JsonRecord }> = [
    { method: "null", properties: { "Chinese Title": null } },
    { method: "archived", properties: { "Chinese Title": { archived: true } } }
  ];
...
async function main() {
  ...
  do {
    ...
    for (const page of response.results as JsonRecord[]) {
      ...
      if (options.apply && plan.action === "migrate" && plan.chineseTitle) {
        await notion.pages.update({
          page_id: plan.pageId,
          properties: {
            "Simplified Chinese Title": { rich_text: richText(plan.chineseTitle) }
          }
        } as never);
  ...
  const archiveAttempt = options.apply && options.archiveProperty && (conflicts.length === 0 || options.forceArchive)
    ? await archiveChineseTitleProperty(notion, dataSourceId)
    : undefined;
