...
async function findDoubanSubject(title, existingInfo, expectedType, cookie) {
  ...
  let candidates = exact.length > 0 ? exact : movieSuggestions.filter((item) => expected.includes(titleKey(item.title)));
...
async function fetchDoubanMetadata(subjectId, cookie) {
  ...
  return {
    ...
    genres,
    imdbId,
    description: ld.description || summary,
...
function buildPatch(page, metadata, imdbRating, posterFile) {
  ...
  if (!hasValue(properties, "简介") && metadata.description) {
    patch["简介"] = { rich_text: richText(metadata.description.slice(0, 1900)) };
  }
  if (!hasValue(properties, "基本信息") && metadata.basicInfo) {
    patch["基本信息"] = { rich_text: richText(metadata.basicInfo.slice(0, 1900)) };
  }
